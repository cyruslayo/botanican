...
import type { ArticleCallout } from "@/lib/types";
import {
  ShoppingBag,
  Tag,
  Megaphone,
...
export default function ArticleCalloutBox({
  ...
  const activeCallout: ArticleCallout | null =
    callout?.enabled !== false && callout?.title
      ? callout
      : fallbackProductSlug
        ? {
            enabled: true,
  ...
  return (
    <div
      id="formulation"
      className="mt-12 p-6 sm:p-8 rounded-2xl bg-surface border border-secondary/30 botanical-shadow flex flex-col sm:flex-row items-center justify-between gap-6 target-glow-effect transition-shadow"
    >
