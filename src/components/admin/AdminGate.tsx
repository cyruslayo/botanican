    ...
    return (
      <div className="max-w-md mx-auto my-8 sm:my-16 bg-surface p-5 sm:p-8 rounded-2xl border border-outline-variant botanical-shadow">
        <div className="mb-6">
          ...
          <h2 className="font-headline-sm text-lg sm:text-headline-sm text-on-surface font-bold mt-0.5">
            Admin Authentication
          </h2>
          <p className="font-body-sm text-xs sm:text-sm text-on-surface-variant mt-1">
            Sign in with authorized administrator credentials to manage
            Botanica.
          </p>
        </div>
        <form onSubmit={handleLogin} className="space-y-4">
          <div className="space-y-1.5">
            <label
              htmlFor="admin-email"
              className="font-label-sm text-xs uppercase tracking-wider text-on-surface-variant font-bold"
            >
              Email
            </label>
            <input
          ...
          {error && (
            <p
              role="alert"
              className="text-error font-body-sm text-xs p-3 bg-error/10 rounded-xl"
            >
              {error}
            </p>
          )}
          <button
            type="submit"
            className="w-full bg-primary text-on-primary py-3 px-4 rounded-xl font-mono text-xs uppercase tracking-wider font-bold hover:bg-primary/90 active:scale-[0.98] transition-all cursor-pointer shadow-xs"
          >
