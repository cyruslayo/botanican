  ...
  const itemSubtotal = Array.isArray(order.items)
    ? sumMoney(
        ...order.items.map((item: any) =>
          multiplyMoney(Number(item.price || 0), Number(item.quantity || 0)),
        ),
      )
    : Math.max(sumMoney(Number(order.total || 0), -shippingFee), 0);
  let allowedStatuses: string[] = [order.status];
  if (order.status === "Pending Verification")
    allowedStatuses = ["Pending Verification", "Processing", "Cancelled"];
  if (order.status === "Processing")
    allowedStatuses = ["Processing", "Shipped"];
  if (order.status === "Shipped") allowedStatuses = ["Shipped", "Fulfilled"];
        ...
        <div className="px-5 py-4 sm:p-6 overflow-y-auto overscroll-contain flex-1 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-surface-container-lowest border border-outline-variant/60 space-y-1">
              ...
              <p className="font-body-md text-primary font-medium text-sm break-all">
                {order.user_id}
              </p>
            </div>
                    ...
                    !order.shipping_address.address && (
                      <>
                        ...
                        <p>
                          ...
                          {order.shipping_address.state || "FCT"}
                        </p>
                      </>
                    )}
                </div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl font-mono text-xs font-bold border transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
                    status === s
