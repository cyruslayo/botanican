...
export default function AdminReferrals() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Membership and referral data could not be loaded from Supabase.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-12 pb-16">
      {error && (
        <div
          role="alert"
          className="p-4 rounded-xl border border-error/30 bg-error/10 text-error font-body-sm text-body-sm"
        >
          {error}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="font-body-md text-body-md text-on-surface-variant">
            Review applicant Instagram handles and phone numbers, approve member
            access, and manage invitation links.
          </p>
        </div>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-xs p-0 sm:p-4 animate-in fade-in">
          <div className="bg-surface p-5 sm:p-6 rounded-t-3xl sm:rounded-2xl border border-outline-variant botanical-shadow max-w-md w-full">
            ...
            <p className="font-body-md text-body-md text-on-surface-variant mb-6">
              Create an invitation code tied to a member's Instagram handle so
              they can invite new applicants.
            </p>
            <form onSubmit={handleCreateCode} className="space-y-4">
              <div>
                <label
                  htmlFor="ownerHandle"
                  className="font-label-sm text-on-surface-variant block mb-1"
                >
                  Member Instagram Handle *
                </label>
                <input
              ...
              <div>
                ...
                <span className="text-[11px] text-on-surface-variant mt-1 block">
                  You can define a custom code name or let the system generate a
                  clean 8-character random code.
                </span>
              </div>
