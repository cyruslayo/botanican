...
const STATUS_COPY: Record<string, string> = {
  "Pending Verification":
    "We received your order and receipt. Payment verification is pending.",
...
export default function OrderHistory() {
  ...
  return (
    <section
      id="order-history"
      className="mt-section-gap scroll-mt-28 border-t border-outline-variant pt-stack-lg"
    >
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-stack-md">
        <div>
          <p className="font-label-sm text-label-sm uppercase tracking-widest text-secondary mb-1">
            Member Orders
          </p>
        ...
        <div className="space-y-stack-md">
          {orders.map((order) => {
            ...
            return (
              <article
                key={order.id}
                className="rounded-xl border border-outline-variant bg-surface-container-low p-5 md:p-6 botanical-shadow"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 pb-4 border-b border-outline-variant/60">
                  ...
                  <div className="sm:text-right">
                    <span
                      className={`inline-flex rounded-full px-3 py-1 font-label-sm text-xs font-semibold ${getStatusClass(order.status)}`}
                    >
                      {order.status}
                    </span>
                ...
                <div className="py-4">
                  <p className="font-body-sm text-on-surface-variant">
                    {STATUS_COPY[order.status] ||
                      "Your order status was updated by Botanica."}
                  </p>
                      ...
                      {ORDER_STEPS.map((step, index) => {
                        ...
                        return (
                          <div key={step} className="min-w-0">
                            <div
                              className={`h-1.5 rounded-full ${complete ? "bg-primary" : "bg-outline-variant"}`}
                            />
                            <p
                              className={`mt-2 text-[10px] sm:text-xs font-label-sm leading-tight ${complete ? "text-primary" : "text-on-surface-variant"}`}
                            >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-body-sm text-on-surface-variant">
                          {item.line_type === "promotion"
                            ? `Bundles ${item.quantity} · ${item.components.map((component) => `${component.quantity_per_bundle} × ${component.name}`).join(" + ")}`
                            : `Qty ${item.quantity}${item.batch_code ? ` · Batch ${item.batch_code}` : ""}`}
                        </p>
                      </div>
                      <p className="font-label-sm text-primary shrink-0">
