...
export async function validateReferralCode(
  code: string,
): Promise<PublicReferralCode | null> {
...
export async function submitAccessRequest(payload: {
  ...
  return {
    success: false,
    error: "The application was not confirmed by Supabase.",
  };
}
...
export async function checkAccess(
  ...
  return {
    ...
    referralCode:
      row.status === "approved" ? row.referral_code || undefined : undefined,
  };
}
...
export async function updateAccessRequest(
  ...
  if (!updated)
    throw new Error("The access request review returned no updated request.");
  return updated;
}
...
export async function createReferralCode(
  ...
  if (!cleanHandle)
    throw new Error("A valid member Instagram handle is required.");
